...
int _mvt_df = 0;
double _mvt_abseps=0.000001;
double _mvt_releps=0;
int _mvt_maxpts=50000;
int _mvt_inform;
double _mvt_error[3];
...
// [[Rcpp::export]]
double pn2(mat y, mat mu, mat sigma) {
  /*int n = y.n_rows;*/
  int k = y.n_elem;
...
*/
vecmat conMuSig(mat sigma, vec mu, vec x, uvec rc1, uvec rc2) {
  vec mu1 = mu.elem(rc1);
  vec mu2 = mu.elem(rc2);
...
/* logfy */
// [[Rcpp::export]]
vec loglik(mat y, mat b, mat u, mat sigma, mat alph, mat dalph, bool cond=0){
  ...
  */
  ...
  for (int i=0; i<n; i++) {
    /* Vectors (used for estimation of conditional mean) */
      vec mu(6) ;
      mu(0) = alph(i,0);
      mu(1) = alph(i,1);
      mu(2) = alph(i,2);
	...
	vecmat out = conMuSig(sigma, mu, x, rc1, rc2);
	vec c_mu = out.V;
	mat c_sig = out.M1;
	...
	/* Marginal dF1_2 */
	/* Conditional mean and variance-covariance matrix, conditional on u1 and u2 */
	vecmat out = conMuSig(sigma, mu, x, rc2, rc4);
	vec c_mu = out.V;
	mat c_sig = out.M1;
	mat ic_sig = c_sig.i(); // the inverse
	double dc_sig = det(c_sig); // the determinant
	...
	vecmat out2 = conMuSig(sigma, mu, x, rc1, rc5);
	vec c_mu2 = out2.V;
	mat c_sig2 = out2.M1;
	mat c_sigX2 = out2.M2;
	...
	/* Conditional F2c1_1 */
	/* Conditional mean and variance-covariance matrix, conditional on alph1_2, u1 and u2 */
	vecmat out3 = conMuSig(sigma, mu, x, rc3, rc5);
	vec c_mu3 = out3.V;
	mat c_sig3 = out3.M1;
	...
	/* Marginal dF1_1 */
	/* Conditional mean and variance-covariance matrix, conditional on u1 and u2 */
	vecmat out = conMuSig(sigma, mu, x, rc1, rc4);
	vec c_mu = out.V;
	mat c_sig = out.M1;
	mat ic_sig = c_sig.i(); // the inverse
	double dc_sig = det(c_sig); // the determinant
	...
	vecmat out2 = conMuSig(sigma, mu, x, rc2, rc5);
	vec c_mu2 = out2.V;
	mat c_sig2 = out2.M1;
	...
	/* Marginal dF2_2 */
	/* Conditional mean and variance-covariance matrix, conditional on u1 and u2 */
	vecmat out = conMuSig(sigma, mu, x, rc3, rc4);
	vec c_mu = out.V;
	mat c_sig = out.M1;
	mat ic_sig = c_sig.i(); // the inverse
	double dc_sig = det(c_sig); // the determinant
	...
	/* Conditional F2c2_1 */
	/* Conditional mean and variance-covariance matrix, conditional on alph2_2, u1 and u2 */
	vecmat out3 = conMuSig(sigma, mu, x, rc2, rc5);
	vec c_mu3 = out3.V;
	mat c_sig3 = out3.M1;
	...
	vecmat out1 = conMuSig(sigma, mu, x, rc1, rc5);
	vec c_mu1 = out1.V;
	mat c_sig1 = out1.M1;
	...
	vecmat out4 = conMuSig(sigma, mu, x, rc4, rc5);
	vec c_mu4 = out4.V;
	mat c_sig4 = out4.M1;
	...
	/* Conditional mean and variance-covariance matrices, conditional on u1 and u2 */
	vecmat out5 = conMuSig(sigma, mu, x, rc6, rc5);
	vec c_mu5 = out5.V;
	mat c_sig5 = out5.M1;

	vecmat out6 = conMuSig(sigma, mu, x, rc7, rc5);
	vec c_mu6 = out6.V;
	mat c_sig6 = out6.M1;

	vecmat out7 = conMuSig(sigma, mu, x, rc8, rc5);
	vec c_mu7 = out7.V;
	mat c_sig7 = out7.M1;

	vecmat out8 = conMuSig(sigma, mu, x, rc9, rc5);
	vec c_mu8 = out8.V;
	mat c_sig8 = out8.M1;
...
/* Dlogfy */
// [[Rcpp::export]]
mat Dloglik(mat y, mat b, mat u, mat sigma, mat alph, mat dalph, bool cond=0){
  /* y: nx2 matrix with event type (0, 1 or 2) of family member 1 and 2
     b: nx4 matrix with XB for event type 1 and 2 (b1 and b2) for family member 1 and 2
     the order is b1_1, b1_2, b2_1 and b2_2
     u: nx2 matrix with the random effects u1 and u2 affecting pi1 and pi2 (cluster-specific risk levels)
...
/* Both family members experience event 2, estimating score contribution from ddF22 */
    ...
    // /* Family member 1 experience event 0, family member 2 experience event 1, estimating score contribution from dF01 */
      ...
      /* Marginal dF1_2 */
      /* Conditional mean and variance-covariance matrix, conditional on u1 and u2 */
      vecmat out1 = conMuSig(sigma, mu, x, rc2, rc4);
      vec c_mu1 = out1.V;
      mat c_sig1 = out1.M1;
      mat c_sigX1 = out1.M2;
    ...
    /* Family member 1 experience event 1, family member 2 experience event 0, estimating score contribution from dF10 */
      ...
      /* Marginal dF1_1 */
      /* Conditional mean and variance-covariance matrix, conditional on u1 and u2 */
      vecmat out1 = conMuSig(sigma, mu, x, rc1, rc4);
      vec c_mu1 = out1.V;
      mat c_sig1 = out1.M1;
      mat c_sigX1 = out1.M2;
      double sd1 = sqrt(c_sig1[0]);
    ...
    /* Family member 1 experience event 0, family member 2 experience event 2, estimating score contribution from dF02 */
      ...
      /* Marginal dF2_2 */
      /* Conditional mean and variance-covariance matrix, conditional on u1 and u2 */
      vecmat out1 = conMuSig(sigma, mu, x, rc3, rc4);
      vec c_mu1 = out1.V;
      mat c_sig1 = out1.M1;
      mat c_sigX1 = out1.M2;
      double sd1 = sqrt(c_sig1[0]);
    ...
    /* Both family members experience event 0, estimating score contribution from F00 */
      ...
      vecmat out9 = conMuSig(sigma, mu, x, rc1, rc11);
      vec c_mu9 = out9.V;
      mat c_sig9 = out9.M1;
      mat c_sigX9 = out9.M2;
      double sd9 = sqrt(c_sig9[0]);

      vecmat out10 = conMuSig(sigma, mu, x, rc2, rc10);
      vec c_mu10 = out10.V;
      ...
      vecmat out11 = conMuSig(sigma, mu, x, rc1, rc13);
      vec c_mu11 = out11.V;
      mat c_sig11 = out11.M1;
      mat c_sigX11 = out11.M2;
      double sd11 = sqrt(c_sig11[0]);

      vecmat out12 = conMuSig(sigma, mu, x, rc4, rc10);
      vec c_mu12 = out12.V;
      ...
      vecmat out13 = conMuSig(sigma, mu, x, rc3, rc11);
      vec c_mu13 = out13.V;
      mat c_sig13 = out13.M1;
      mat c_sigX13 = out13.M2;
      double sd13 = sqrt(c_sig13[0]);

      vecmat out14 = conMuSig(sigma, mu, x, rc2, rc12);
      vec c_mu14 = out14.V;
      ...
      vecmat out15 = conMuSig(sigma, mu, x, rc3, rc13);
      vec c_mu15 = out15.V;
      mat c_sig15 = out15.M1;
      mat c_sigX15 = out15.M2;
      double sd15 = sqrt(c_sig15[0]);

      vecmat out16 = conMuSig(sigma, mu, x, rc4, rc12);
      vec c_mu16 = out16.V;
    ...
    if (cond) {
      ...
      double dpdfu_u1 = pdfu*(-0.5)*(2*sigu(1,1)*pu(0)-sigu(0,1)*pu(1)-sigu(1,0)*pu(1))/denom;
      double dpdfu_u2 = pdfu*(-0.5)*(2*sigu(0,0)*pu(1)-sigu(0,1)*pu(0)-sigu(1,0)*pu(0))/denom;

      /* Adding to score */
      res(i,0) += (1/pdfu)*dpdfu_u1;
      res(i,1) += (1/pdfu)*dpdfu_u2;
    }
  }
  return(res);
}
...
/* Hes */
// [[Rcpp::export]]
mat Hes(mat y, mat b, mat u, mat sigma, mat alph, mat dalph, bool cond=0){
  ...
  */
  ...
  res.row(0) = (Dloglik(y, b, u1_p, sigma, alph, dalph, cond)-Dloglik(y, b, u1_m, sigma, alph, dalph, cond))/(2*h);
  res.row(1) = (Dloglik(y, b, u2_p, sigma, alph, dalph, cond)-Dloglik(y, b, u2_m, sigma, alph, dalph, cond))/(2*h);
